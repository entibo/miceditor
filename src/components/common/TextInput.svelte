<script>
  ...
  function onInput(e) {
    ...
    if (color) {
      M.andThen(Platform.readColor(str), (v) => (newValue = v))
    } else if (int || float) {
  ...
  $: presentationValue = focused
    ...
    ? ""
    : float
    ? Math.round(internalValue * 100) / 100
    : int
...
<Tooltip noStyle top active={hasTooltip && focused} class={outerClass}>
  {#if preview && (color || platform)}
    <div class="preview-container w-6 h-6 flex items-center justify-center">
      {#if color}
        <div
          class="preview rounded-full"
          style="background: #{internalValue}"
        />
      {:else if platform && internalValue !== undefined && internalValue !== null}
        <img
          class="preview shadow"
          src="dist/grounds/{Platform.typeNames[internalValue]}.png"
          alt={Platform.typeNames[internalValue]}
        />
      {/if}
    </div>
  {/if}

  <div class="material-input {className}" class:disabled class:invalid>
    <input
      type="text"
  ...
  >
    {#if color}
      <ColorPicker
        ...
        on:input={() => setNewValue(internalValue)}
        on:input
      />
    {:else if platform}
      <div class="flex flex-wrap justify-center p-2 ">
        {#each Platform.typeNames as name, type}
          <div
            class="tile dim-40"
            class:active={type == internalValue}
    ...
    {:else if shamanObject}
      <div class="p-2 ">
        {#if shamanObjectMetadata.get(internalValue || 0).variants}
          <ShamanObjectVariantsMenu
            typeProp={internalValue}
            onSelect={(type) => setNewValue(type)}
          />
        {:else}
          <ShamanObjectsMenu
            typeProp={internalValue}
