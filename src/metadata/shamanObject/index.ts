...
export type Metadata = (Sprite | Spritesheet) & {
...
export default {
  ...
  /** Will work as expected when used in a map */
  mapEntries: () =>
    [...data.entries()].filter(
      ([type]) =>
        type !== 26 &&
  ...
  /** Will work as expected when used in shaman_tools */
  shamanToolsEntries: () =>
    [...data.entries()].filter(
      ([type]) =>
        ![0, 25, 39, 40, 45, 46, 54, 57, 60, 61, 62, 67, 68, 69, 70].includes(
          type
        )
    ),
}
...
function normalizeDimensions<T extends Dimensions>(value: T): T {
  if (value.boundingWidth === undefined) value.boundingWidth = value.width
  if (value.boundingHeight === undefined) value.boundingHeight = value.height
  return value
}

function setData(
  types: number | number[],
  arg: Metadata | ((t: number) => Metadata)
) {
...
setData([1, ...range(101, 150)], (type) => {
  let variant = type === 1 ? 0 : type - 100
  return {
    variants: range(101, 150),
    isVariant: type !== 1,
    spritesheet: "small-boxes.png",
    width: 70,
    height: 70,
    offset: { x: 70 * variant, y: 0 },
    boundingWidth: 30,
    boundingHeight: 30,
    placeholderData: {
      baseId: 1,
...
setData([2, ...range(201, 250)], (type) => {
  let variant = type === 2 ? 0 : type - 200
  return {
    variants: range(201, 250),
    isVariant: type !== 2,
...
setData([3, ...range(301, 350)], (type) => {
  let variant = type === 3 ? 0 : type - 300
  return {
    variants: range(301, 350),
    isVariant: type !== 3,
    spritesheet: "small-planks.png",
    width: 220,
    height: 70,
    offset: { x: 220 * variant, y: 0 },
    boundingWidth: 100,
    boundingHeight: 10,
    placeholderData: {
      baseId: 3,
...
setData([4, ...range(401, 450)], (type) => {
  let variant = type === 4 ? 0 : type - 400
  return {
    variants: range(401, 450),
    isVariant: type !== 4,
    spritesheet: "big-planks.png",
    width: 320,
    height: 80,
    offset: { x: 320 * variant, y: 0 },
    boundingWidth: 200,
    boundingHeight: 10,
    placeholderData: {
      baseId: 4,
...
setData([6, ...range(601, 645)], (type) => {
  let variant = type === 6 ? 0 : type - 600
  return {
    variants: range(601, 645),
    isVariant: type !== 6,
    spritesheet: "balls.png",
    width: 80,
    height: 80,
    offset: { x: 80 * variant, y: 0 },
    boundingWidth: 30,
    boundingHeight: 30,
    circle: true,
    placeholderData: {
...
setData([7, ...range(701, 716)], (type) => {
  let variant = type === 7 ? 0 : type - 700
  return {
    variants: range(701, 716),
    isVariant: type !== 7,
    spritesheet: "trampolines.png",
    width: 150,
    height: 100,
    offset: { x: 150 * variant, y: 0 },
    boundingWidth: 100,
    boundingHeight: 20,
    placeholderData: {
      baseId: 7,
...
setData([10, ...range(1002, 1030)], (type) => {
  let variant = type === 10 ? 0 : type - 1000 - 1
  return {
    variants: range(1002, 1030),
    isVariant: type !== 10,
...
setData([17, ...range(1701, 1740)], (type) => {
  let variant = type === 17 ? 0 : type - 1700
  return {
    variants: range(1701, 1740),
    isVariant: type !== 17,
...
setData([...range(28, 31), ...range(2801, 2850)], (type) => {
  let variant = type >= 28 && type <= 31 ? type - 28 : type - 2801 + 4
  return {
    variants: [...range(29, 31), ...range(2801, 2850)],
    isVariant: type !== 28,
